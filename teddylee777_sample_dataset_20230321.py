...
def split_dataset1(folders, test_size=0.2):
    # train / test 셋의 파일을 나눕니다.
    train_images = []
    test_images = []
    ...
    train_labels = [f.split('\\')[-2] for f in train_labels]
    test_labels = [f.split('\\')[-2] for f in test_labels]
...
def cats_and_dogs1(train_transform, test_transform, test_size=0.2):
    # download_url = 'https://download.microsoft.com/download/3/E/1/3E1C3F21-ECDB-4869-8368-6DEBA77B919F/kagglecatsanddogs_5340.zip'
    # folders = download_dataset(download_url, folder='PetImages')

    folders = ['tmp/PetImages/Cat', 'tmp/PetImages/Dog']
    (train_images, train_labels), (test_images, test_labels), class_to_idx = split_dataset1(folders, test_size=test_size)
